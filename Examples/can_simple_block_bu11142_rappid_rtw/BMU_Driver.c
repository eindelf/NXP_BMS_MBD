...
static void SPI_Tx_Dummy(uint8_t *data, uint8_t len, uint8_t *rcv_data, uint8_t
  rcv_len, uint16_t delay, uint8_t EndCs)
{
  DSPI_PUSHR_32B_tag pushr;
  uint8_t i;
  uint16_t temp;
  ...
  for (i = 0; i < len-1; i++) {
    ...
    //rcv_data[i] = (uint8_t)DSPI[0]->SR.B.RXCTR;                 /* Read data received by master SPI */
