...
#include "dspi_564xL_library.h"
  ...
  {
    uint16_t ret = 0U;
    while (DSPI[dspi_num]->SR.B.RFDF != 1) {
  ...
  void dspi_564xl_pin_init(uint8_t pcr_num, uint8_t in_out, uint8_t dspi_num)
  {
    ...
    } else if (pcr_num == 1) {         /* PCR[1]	DSPI_2 SOUT ALT2 — —  */
      SIU.PCR[pcr_num].R = 0x0A00;     /*   out    */
    } else if (pcr_num == 2) {         /* PCR[2]	DSPI_2 — — SIN PSMI[2]; PADSEL=0  */
      SIU.PCR[pcr_num].R = 0x0100;     /*   in    */
      SIU.PSMI[2].B.PADSEL = 0;
    } else if (pcr_num == 3) {         /*  PCR[3]	DSPI_2 CS0 ALT2 CS0 PSMI[3]; PADSEL=0  */
      if (in_out == 0) {               /*   in    */
        SIU.PCR[pcr_num].R = 0x0100;   /*   in    */
    ...
    } else if (pcr_num == 4) {         /*  PCR[4]	DSPI_2 CS1 ALT2 — — */
      SIU.PCR[pcr_num].R = 0x0A00;     /*   out    */
    } else if (pcr_num == 5) {
      if (dspi_num == 0) {             /*  PCR[5]	DSPI_0 CS7 ALT3 — — */
        SIU.PCR[pcr_num].R = 0x0E00;   /*   out    */
      } else if (dspi_num == 1) {      /*  PCR[5]	DSPI_1 CS0 ALT1 CS0 — */
        if (in_out == 0) {
          SIU.PCR[pcr_num].R = 0x0100; /*   in    */
        } else if (in_out == 1) {
    ...
    } else if (pcr_num == 6) {         /*  PCR[6]	DSPI_1 SCK ALT1 SCK — */
      if (in_out == 0) {               /*   in    */
        SIU.PCR[pcr_num].R = 0x0100;
      } else if (in_out == 1) {        /*   out    */
        SIU.PCR[pcr_num].R = 0x0600;
      }
    } else if (pcr_num == 7) {         /*  PCR[7]	DSPI_1 SOUT ALT1 — — */
      SIU.PCR[pcr_num].R = 0x0600;     /*   out    */
    } else if (pcr_num == 8) {         /*  PCR[8]	DSPI_1 — — SIN — */
      SIU.PCR[pcr_num].R = 0x0100;     /*   in    */
    } else if (pcr_num == 9) {         /*  PCR[9]	DSPI_2 CS1 ALT1 — — */
      SIU.PCR[pcr_num].R = 0x0600;     /*   out    */
    } else if (pcr_num == 10) {        /*  PCR[10]	DSPI_2 CS0 ALT1 CS0 PSMI[3]; PADSEL=1 */
      if (in_out == 0) {               /*   in    */
        SIU.PCR[pcr_num].R = 0x0100;
    ...
    } else if (pcr_num == 11) {        /*  PCR[11]	DSPI_2 SCK ALT1 SCK PSMI[1]; PADSEL=1  */
      if (in_out == 0) {               /*   in    */
        SIU.PCR[pcr_num].R = 0x0100;
    ...
    } else if (pcr_num == 12) {        /*  PCR[12]	DSPI_2 SOUT ALT1 — —   */
      SIU.PCR[pcr_num].R = 0x0600;
    } else if (pcr_num == 13) {        /*  PCR[13]	DSPI_2 — — SIN PSMI[2]; PADSEL=1   */
      SIU.PCR[pcr_num].R = 0x0100;
      SIU.PSMI[2].B.PADSEL = 1;
    } else if (pcr_num == 22) {        /*  PCR[22]	DSPI_2 CS2 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    } else if (pcr_num == 36) {        /*  PCR[36]	DSPI_0 CS0 ALT1 CS0 —   */
      if (in_out == 0) {               /*   in    */
        SIU.PCR[pcr_num].R = 0x0100;
      } else if (in_out == 1) {        /*   out    */
        SIU.PCR[pcr_num].R = 0x0600;
      }
    } else if (pcr_num == 37) {        /*  PCR[37]	DSPI_0 SCK ALT1 SCK —   */
      if (in_out == 0) {               /*   in    */
        SIU.PCR[pcr_num].R = 0x0100;
      } else if (in_out == 1) {        /*   out    */
        SIU.PCR[pcr_num].R = 0x0600;
      }
    } else if (pcr_num == 38) {        /*  PCR[38]	DSPI_0 SOUT ALT1 — —   */
      SIU.PCR[pcr_num].R = 0x0600;
    } else if (pcr_num == 39) {        /*  PCR[39]	DSPI_0 — — SIN —   */
      SIU.PCR[pcr_num].R = 0x0100;
    } else if (pcr_num == 42) {        /*  PCR[42]	DSPI_2 CS2 ALT1 — —   */
      SIU.PCR[pcr_num].R = 0x0600;
    } else if (pcr_num == 43) {        /*  PCR[43]	DSPI_2 CS2 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    } else if (pcr_num == 44) {        /*  PCR[44]	DSPI_2 CS3 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    } else if (pcr_num == 53) {        /*  PCR[53]	DSPI_0 CS3 ALT1 — —   */
      SIU.PCR[pcr_num].R = 0x0600;
    } else if (pcr_num == 54) {        /*  PCR[54]	DSPI_0 CS2 ALT1 — —   */
      SIU.PCR[pcr_num].R = 0x0600;
    } else if (pcr_num == 55) {
      if (dspi_num == 0) {             /*  PCR[55]	DSPI_0 CS4 ALT3 — —   */
        SIU.PCR[pcr_num].R = 0x0E00;   /*   out    */
      } else if (dspi_num == 1) {      /*  PCR[55]	DSPI_1 CS3 ALT1 — —   */
        SIU.PCR[pcr_num].R = 0x0600;   /*   out    */
      }
    } else if (pcr_num == 56) {
      if (dspi_num == 0) {             /*  PCR[56]	DSPI_0 CS5 ALT3 — —   */
        SIU.PCR[pcr_num].R = 0x0E00;   /*   out    */
      } else if (dspi_num == 1) {      /*  PCR[56]	DSPI_1 CS2 ALT1 — —   */
        SIU.PCR[pcr_num].R = 0x0600;   /*   out    */
      }
    } else if (pcr_num == 77) {        /*  PCR[77]	DSPI_2 CS3 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    } else if (pcr_num == 79 ) {       /*  PCR[79]	DSPI_0 CS1 ALT1 — —   */
      SIU.PCR[pcr_num].R = 0x0600;
    } else if (pcr_num == 83) {        /*  PCR[83]	DSPI_0 CS6 ALT1 — —   */
      SIU.PCR[pcr_num].R = 0x0600;
    } else if (pcr_num == 98) {        /*  PCR[98]	DSPI_1 CS1 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    } else if (pcr_num == 101) {       /*  PCR[101]	DSPI_2 CS3 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    } else if (pcr_num == 104) {       /*  PCR[104]	DSPI_0 CS1 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    } else if (pcr_num == 105) {       /*  PCR[105]	DSPI_1 CS1 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    } else if (pcr_num == 106) {       /*  PCR[106]	DSPI_2 CS3 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    } else if (pcr_num == 117) {       /*  PCR[117]	DSPI_0 CS4 ALT3 — —   */
      SIU.PCR[pcr_num].R = 0x0E00;
    } else if (pcr_num == 118) {       /*  PCR[118]	DSPI_0 CS5 ALT3 — —   */
      SIU.PCR[pcr_num].R = 0x0E00;
    } else if (pcr_num == 120) {       /*  PCR[120]	DSPI_0 CS6 ALT3 — —   */
      SIU.PCR[pcr_num].R = 0x0E00;
    } else if (pcr_num == 121) {       /*  PCR[121]	DSPI_0 CS7 ALT3 — —   */
      SIU.PCR[pcr_num].R = 0x0E00;
    } else if (pcr_num == 128) {       /*  PCR[128]	DSPI_0 CS4 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    } else if (pcr_num == 129) {       /*  PCR[129]	DSPI_0 CS5 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    } else if (pcr_num == 130) {       /*  PCR[130]	DSPI_0 CS6 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    } else if (pcr_num == 131) {       /*  PCR[131]	DSPI_0 CS7 ALT2 — —   */
      SIU.PCR[pcr_num].R = 0x0A00;
    }
