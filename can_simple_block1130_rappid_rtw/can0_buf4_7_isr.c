...
void can0_buf4_7_isr(void)
{
  ...
  if (isr_mask & 1<<5) {
    if (flexcan_564xl_rxbuffull( 0, 5)) {
      ...
      can_simple_block1130_B.CANReceiveDataTrigger1_o2 = flexcan_564xl_get_id(0,
        5);
      flexcan_564xl_get_mb(ptr_FLEXCAN_MsgBufferStructure, 0, 5);
      for (i=0;i<ptr_FLEXCAN_MsgBufferStructure->length;i++) {
        (&can_simple_block1130_B.CANReceiveDataTrigger1_o4[0])[i] =
          ptr_FLEXCAN_MsgBufferStructure->data[i];
      }

      can_simple_block1130_B.CANReceiveDataTrigger1_o3 =
        flexcan_564xl_get_idtype(0, 5);
      can_simple_block1130_B.CANReceiveDataTrigger1_o5 = flexcan_564xl_get_ts(0,
        5);
      flexcan_564xl_release_mb_lock(0);
      flexcan_564xl_clear_mask(0, 5);
      CAN_0.IFLAG1.R = 1<<5;
      can_simple_block1130_Subsystem();
    }
  }
}
