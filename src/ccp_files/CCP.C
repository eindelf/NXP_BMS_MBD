...
void ccpCommand( CCP_BYTEPTR com ) {
  ...
  /* Handle other commands only if connected */
    ...
    switch (cmd) {
	  ...
	  case CC_DIAG_SERVICE: /* Set Diagnostic service */
	  	{
	  		#define dig_service com[2]
	  		ccp.Crm[3] = 0x1;	  		
	  		ccp.Crm[4] = 0x0;
	  			  		
	  		if(dig_service == 0x08)
	  		{	  			  			
	  			ccp.Crm[4] = (CCP_BYTE)JumpToBootSector((CCP_MTABYTEPTR)(*(CCP_DWORD *)&com[3]));
	  			/* CallBack to get the app boot-sector and jump to downloaded app */
	  		}	  		
	  	
          #ifdef CCP_TESTMODE
            if (gDebugLevel) CCPPRINT("%u: CC_DIAG_SERVICE -> major=%02X minor=%02X\n",ctr,ccp.Crm[3],ccp.Crm[4]);
          #endif
